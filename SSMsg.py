...
class Gui(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle('SmartSchool Messages') #le titre de la fenêtre principale
        #centre la fenêtre
        ...
        #fin du centrage
        self.setWindowIcon(QtGui.QIcon('./icons/ssmsg.svg')) #attribue une icone à la fenêtre principale
        ...
        importFilesAction = QtWidgets.QAction(QtGui.QIcon('./icons/import_file.svg'),"&Sélectionner les fichiers des à envoyer", self)
        ...
        quitAction = QtWidgets.QAction(QtGui.QIcon('./icons/quit.svg'),"Quitter", self)
        ...
        aboutAction = QtWidgets.QAction(QtGui.QIcon('./icons/ssmsg.svg'),"Á propos", self)
        ...
        helpAction = QtWidgets.QAction(QtGui.QIcon('./icons/help.svg'),"Obtenir de l'aide", self)
        ...
        configAction = QtWidgets.QAction(QtGui.QIcon('./icons/config.svg'),"Configuration", self)
        ...
        refreshAction = QtWidgets.QAction(QtGui.QIcon('./icons/refresh.svg'),"Recharger liste élèves", self)
        refreshAction.setStatusTip("Recharger le dictionnaire depuis SmartSchool")
        ...
        dockWidgetMessage.setTitleBarWidget(QtWidgets.QLabel( '<p style="font-size:11pt;font-weight:bold">Message</p>' ))
        dockWidgetMessage.setFeatures(QtWidgets.QDockWidget.NoDockWidgetFeatures)
        ...
        self.boutton_ok=QtWidgets.QPushButton(QtGui.QIcon('./icons/ok_apply.svg'),'Envoyer')
        ...
        self.sendComptePrincipal.setToolTip ('<p>Envoyer le fichier sur le compte principal</p>')
        self.sendComptePrincipal.setCheckState(0)
        self.sendComptePrincipal.stateChanged.connect(self.changeDestinataire)
        self.sendComptesSecondaires=QtWidgets.QCheckBox("Comptes secondaires")
        self.sendComptesSecondaires.setToolTip (('<p>Envoyer le fichier sur les comptes secondaires 1 et 2</p>'))
        self.sendComptesSecondaires.setCheckState(2)
        self.sendComptesSecondaires.stateChanged.connect(self.changeDestinataire)
    ...
    def prodDictEleves(self):
        ...
        try:
            ...
            prog.setWindowFlags(QtCore.Qt.SplashScreen | QtCore.Qt.WindowStaysOnTopHint)
            prog.setMinimumWidth(350)
            
            prog.setCancelButton(None)
            prog.setLabel (QtWidgets.QLabel("<p>Récupération des élèves sur Smartschool</p><p>Veuillez patienter...</p>"))
            for i in range(50):
                prog.setValue(i)
                prog.show()
                time.sleep(0.01) #ce temps d'attente ne sert à rien mais il évite de voir la barre de progression s'afficher directement à 49%
            ...
            result=client.service.getAllAccountsExtended(self.dictConfig['SSApiKey'],self.dictConfig['gpEleves'],1)
            ...
            for i in range(51):
                prog.setValue(i+50)
                QtWidgets.QApplication.processEvents()
                time.sleep(0.01)#ce temps d'attente ne sert à rien mais il évite de voir la barre de progression disparaitre subitement
            
        except Exception as e:
            
            #self.dialPatientez.close()
            QtWidgets.QMessageBox.warning(self,'Erreur récupération élèves',"<p>Il n'a pas été possible de récupérer la liste des élèves sur SmartSchool. </p><p>Vérifiez la <b>configuration</b> du logiciel et refaites une tentative. </p><p>Si cette erreur persiste, contactez le développeur.</p>")
            print ('Erreur : %s' % e)
    ...
    def changeDestinataire(self):
        """
        Modifie les valeurs de sstatutMsgComptePrincipal, statutMsgCoaccount1 et statutMsgCoaccount2 en fonction des valeurs des checkbox sendComptePrincipal et sendComptesSecondaires
        """
        for eleve in self.dictEleves.values():
    ...
    def verifAvantImport(self):
        ...
        if valide==0:
            ...
            try:
                ...
                QtWidgets.QMessageBox.warning(self,'Erreur critique',"<p>Une erreur critique s'est produite, contactez le développeur.</p>")
                print ('Erreur : %s' % e)
                print ('Message : ', traceback.format_exc())
    #
    def importFilesDialog(self):
        ...
        """
        listFilesNames = QtWidgets.QFileDialog.getOpenFileNames(self,'Les fichiers à envoyer',self.current_dir,filter='PDF files (*.pdf)')
        for filePath in listFilesNames[0]:
            try:
                
                fileName=os.path.basename(filePath) #récupère le nom du fichier avec son extension
                fileName=os.path.splitext(fileName)[0]
                self.dictEleves[fileName].filePath=filePath #à chaque élève du dictEleves pour lequel il y a un fichier, j'ajoute le path vers ce fichier
            except KeyError:
                alert="<div><p>Aucune correspondance n'a été trouvée pour le fichier avec le numéro %s</p>"%(os.path.splitext(fileName)[0])
                ...
                QtWidgets.QMessageBox.warning(self,'Aucune correspondance',alert)
        self.updateFileList()
    ...
    def updateFileList(self):
        ...
        self.dockWidgetList.setTitleBarWidget(QtWidgets.QLabel( '<p style="font-size:10pt;font-weight:bold">Fichiers</p>' ))
        self.dockWidgetList.setFeatures(QtWidgets.QDockWidget.NoDockWidgetFeatures)
        ...
        self.tableWidget.setHorizontalHeaderLabels(['Nom du fichier',"Nom et prénom de l'élève","Statut compte principal","Statut compte sec. 1","Statut compte sec. 2"])
        ...
        for eleve in self.dictEleves.values():
            if eleve.filePath!="":
                ...
                itemStatutMsgPrinc=QtWidgets.QTableWidgetItem(eleve.statutMsgComptePrincipal)
                itemStatutMsg1=QtWidgets.QTableWidgetItem(eleve.statutMsgCoaccount1)
                itemStatutMsg2=QtWidgets.QTableWidgetItem(eleve.statutMsgCoaccount2)
                
                self.tableWidget.insertRow(self.tableWidget.rowCount())
                self.tableWidget.setItem(self.tableWidget.rowCount()-1,0,itemFileName)
                self.tableWidget.setItem(self.tableWidget.rowCount()-1,1,itemNomPrenom)
                self.tableWidget.setItem(self.tableWidget.rowCount()-1,2,itemStatutMsgPrinc)
                self.tableWidget.setItem(self.tableWidget.rowCount()-1,3,itemStatutMsg1)
                self.tableWidget.setItem(self.tableWidget.rowCount()-1,4,itemStatutMsg2)
        
        #creation du bouton remove
        self.boutton_remove=QtWidgets.QPushButton(QtGui.QIcon('./icons/remove.svg'),'Retirer de la liste')
    ...
    def openDialConfig(self):
        ...
        if self.dictConfig["SSApiKey"]!="":
            ...
            self.saisieApiKey.setPlaceholderText("Le code d'accèsà l'API de SmartSchool")
        mon_layout.addWidget(self.saisieApiKey)
        
        mon_label=QtWidgets.QLabel("<p>Numéro interne SS de l'expéditeur</p>") 
        mon_layout.addWidget(mon_label)
        
        self.saisieInternummer=QtWidgets.QLineEdit()
        if self.dictConfig["interNummerExpediteur"]!="":
            self.saisieInternummer.setText(self.dictConfig["interNummerExpediteur"])
        else:
            self.saisieInternummer.setPlaceholderText("Numéro interne SmartSchool de l'expéditeur")
        mon_layout.addWidget(self.saisieInternummer)
        
        mon_label=QtWidgets.QLabel("<p>Groupe des élèves dans SmartSchool</p>") 
        mon_layout.addWidget(mon_label)
        ...
        self.dialConfig.setLayout(mon_layout) #ajout du layout dans la fenetre
    ...
    def sendMsg(self,eleve):
        ...
        if self.sendComptePrincipal.isChecked():
            result0=client.service.sendMsg(self.dictConfig["SSApiKey"],toaddr,subject,body,fromaddr,jsonAttachment,0,0) #
            if result0==0:
        ...
        if self.sendComptesSecondaires.isChecked():
            if eleve.status1=="actief":
                result1=client.service.sendMsg(self.dictConfig["SSApiKey"],toaddr,subject,body,fromaddr,jsonAttachment,1,0) #
                if result1==0:
            ...
            if eleve.status2=="actief":
                result2=client.service.sendMsg(self.dictConfig["SSApiKey"],toaddr,subject,body,fromaddr,jsonAttachment,2,0) #
                if result2==0:
        ...
        if result0==0 & result1==0 & result2==0: #si aucun problème n'a été rencontré dans l'envoi des messages
            # get the path to the file in the current directory
            src = os.path.dirname(eleve.filePath) #on récupère le path du fichier
            # rename the original file
            newName = os.path.join(src, attachmentName) #on produit le nouveau nom du fichier 
    ...
    def sendAllFiles(self):
        ...
        if valide==0:
            QtWidgets.QMessageBox.warning(self,'Pas de sujet - pas de message',alert)
        else:
    ...
    def about(self):
        ...
        message+="<p> <b>Language : </b>Python %s</p>  <p> <b>Interface : </b>Qt %s</p> <p> Pour signaler un bug ou proposer une amélioration, <br/>" %(platform.python_version(),QtCore.QT_VERSION_STR)
